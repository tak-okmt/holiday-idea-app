...
### 2026-09-23 カタログの拡充と質改善（週末8の前に）
- 開発者から「提案の種類の少なさ」「内容自体の改善余地」の指摘。サブエージェント（Opus・Web検索あり）にリサーチと実装を依頼し、開発者がレビューしたうえで反映。
- **200件 → 272件に拡充**（新規72件は`scripts/generate-catalog.ts`の`items`配列末尾に追記。既存200件のidは配列位置を変えていないため一切ズレていない）。
  - 偏り是正（`catalog:analyze`実測、週末1・週末6からの継続課題）: `with=family` 32.5%→**41.5%**、`with=friends` 44.5%→**48.9%**、`energy=high` 13.0%→**15.4%**、`budget=unlimited` 8.0%→**12.9%**。category配分は10.7〜11.8%でほぼ均等を維持。
  - 週末6で決めた「craft/food/appreciation/learning/restのようなenergy=lowが自然なカテゴリを無理にhighへ寄せない」方針は継続。`energy=high`の新規項目はnature/sports/playに限定して正直に付与。
  - 新規項目の例: ピックルボール・フットゴルフ・モルック・ラフティング・斧投げ（friends×high）、貸切サウナ・日帰りグランピング・ホテルのデイユース（unlimited）、家族の昔話の記録・毛布の秘密基地・流しそうめん（family×free/低予算）など。Web検索で日本の20〜50代の余暇トレンド（趣味ランキング、2026年注目のニュースポーツ、世代別の傾向）を調査し、具体的な店舗・施設名は含めずコンセプト単位で反映した。
  - 既存18件（例: act_031, act_129, act_145, act_188 など）は内容の重複・矛盾（名称重複、energyとの不整合、pitch/first_step冒頭の重複）を理由に、id・配列位置を変えずに文言のみ書き直した。
  - `pnpm test`（104件）/ `pnpm run lint` / `tsc --noEmit` すべて通過を確認（開発者が独自に再実行して一致確認済み）。
- CLAUDE.mdのロードマップに対応する項目はなし（週末7と週末8の間の追加改善のため、チェックボックスは追加しない）。

### 2026-09-23 カタログ拡充の効果検証（外注テストの前に）
- 友人テスト・外注テストの前に、拡充が実際に効いているかを無料・即日で確認したいという開発者の意向を受け、`scripts/evaluate-coverage.ts`を新規作成（`pnpm run catalog:coverage -- --before=<旧カタログのパス> --after=<新カタログのパス、省略時はdata/activities.json>`）。
  - with×energy×time×budgetの全108通りをJevを使わずルールスコアのみで機械的に評価するツール。デッドエンド数・候補数平均/最小・カバレッジ（top3に一度でも入る活動の割合）を算出する。
  - **重要な発見**: `ruleMatchScore`(`src/engine/rules.ts`)はenergyの一致度のみでスコアを決めるため、最初の提案では同点(1.0)が大量発生し、安定ソートによりカタログ配列の並び順(＝古いidほど有利)で勝敗が決まることが判明。ルールのみのオフライン評価では新規72件がほぼ浮上せず、カバレッジは19.5%(39/200件)→14.7%(40/272件)で改善しなかった。ただしこれは**拡充が効いていないのではなく、Jev障害時のフォールバックに位置バイアスがある**という、カタログの中身とは別軸の設計上の発見。将来Jevが使えない状況が続く場合、新しい候補がほぼ出てこないリスクとして記録しておく（対応は今回のスコープ外、未着手）。
  - 本番と同じJevで24ケース（with/energy/budgetの代表的な組み合わせ×気分の自由記述）を実際に採点させたところ、新規72件がtop3に入ったケースは8/24件(33.3%)、全72スロット中13個(18.1%)。特に`family×high×under3000`「子どもと体を動かしたい」、`family×medium×under3000`「子どもが夢中になれることを」ではtop3が全て新規項目となり、**狙って厚くしたfamily・子ども向けの軸は実際にJevで選ばれるようになっていることを確認**。`friends×high×unlimited`のような一部の組み合わせは既存の定番（テニス・フットサル等）が依然強く、拡充の効果は軸によってムラがある。
  - この検証により、開発者が検討していたクラウドワークスでの外注テストを待たずに「拡充が効いているか」という当初の問いにはある程度答えが出せた。
  - `scripts/evaluate-coverage.ts`は今後カタログを変更するたびに再利用できるツールとしてリポジトリに残す。

### 2026-09-23 friends×unlimitedニッチの追加改善
- 上記の24ケース検証で「`friends×unlimited`は既存の定番(テニス・フットサル等)が依然強い」とした点について、本当に構造的な偏りかサンプル不足かを切り分けるため、`friends×budget=unlimited`に絞って20ケース×2回、実際のJevで再検証した。
  - **構造的な偏りと判定**: energy=highの12ケースは2回とも新規項目のtop3占有が**0/36(0%)**。原因は3点特定できた。①`ruleMatchScore`(`src/engine/rules.ts`)はenergyの近さしか見ないため、`budget=unlimited`を選んでもルールスコアは一切上がらず、低予算の定番と対等以下でしか戦えない。②既存の勝ち組項目は活動名自体に社会性シグナルがある（「友人と打ち合う」「仲間内で楽しむ」）のに対し、新規項目は活動名のみで社会性を書いていなかった。③新規項目のpitch文にあった「〜でなくても」という留保表現が、高気力・社交的な気分に対して弱さとして働いていた。
  - 対応として、`with=["friends"], energy="high", budget="unlimited"`向けに5件追加（act_273〜277、配列末尾に追記、既存272件は1件も変更なし）。活動名に「友人と／仲間と」を明記し、pitch文は留保表現を避けて社会性・身体性を無留保で言い切る書き方に統一。
  - 再検証（before/after、各2回）: energy=highスロットの新規項目占有率が **0%→50%(18/36)** に改善。
  - **追加した5件のうち act_276(ゴルフコースを仲間と一日かけて回る) だけは再検証後もtop3に一度も入らなかった**。開発者に報告のうえ、現状はそのまま残す判断（カテゴリの幅として許容）。
  - **技術的な発見（要記録）**: Jevは候補全件を1リクエストでまとめて採点する設計(`judge.ts`の`scoreCandidates`)のため、カタログに項目を追加すると**既存項目のJevスコアも連動して変動する**（例: 変更していないact_266のjevスコアが検証のたびに0.58〜0.85の間で動いた）。そのため「0%→50%」の改善は追加5件だけの純粋な効果ではなく、一部は既存項目の再較正を含む数字である点に注意。今後Jevベースの前後比較を行う際は、この非独立性を踏まえて解釈すること。

### 2026-09-23 budgetがJevの採点に渡っていなかったバグを修正
- 「soloでbudgetをunder3000→unlimitedに変えても提案が変わらない」という開発者の指摘を受けて調査。実際に予算3段階のみを変えて実Jevで15パターン×3段階(計120リクエスト)を比較した。
  - **原因はバグと判定**: `judge.ts`の`scoreCandidates`がJevに送る候補データ(`candidates.map(...)`)に**`budget`フィールドが含まれていなかった**。プロンプト自体は「`profile`（誰と/気力/使える時間/予算/季節/天気）を踏まえて」と明記し`state.profile.budget`は渡っているのに、各候補がいくらかかるかは一度もJevに伝えていなかった。Jevは名前やpitchの文面から予算感を推測するしかなく、budget段階別のJevスコア平均差はわずか+0.028しかなかった。
  - `rules.ts`のbudgetが上限の足切りのみでスコアに影響しない点、`ruleMatchScore`がenergyのみでbudgetを見ない点は**意図された設計**（週末3・週末4からの一貫した方針）であり、修正対象ではないと判断。
  - **修正**: `src/engine/judge.ts`の候補ペイロードに`budget: c.budget`を1行追加。Jev稼働時のみ効く修正で、Jev障害時のフォールバック(カタログ配列順で同点が決まる別件の課題)には影響しない。
  - 修正後の再検証(2サンプル): top3内でbudget=unlimited活動が占める割合が**6.7%→16.7%**に改善（カタログ全体に占めるunlimitedの割合12.9%とほぼ同等〜やや上回る水準に）。mood無しケースでも0/30→3/30で改善が確認できた。開発者も別途3ケースで実機確認し再現を確認済み。
  - **不採用にした代替案**: `ruleMatchScore`に予算一致度を加点する案も同じデータでオフライン試算したが、わずかな重みでも過剰補正（under3000ユーザーへの無料候補の露出が急減、「お金がかかる」却下後の減点方向とも矛盾）となったため見送った。
  - `pnpm test`(104件)/`lint`/`tsc --noEmit`すべて通過。開発者が`src/engine/judge.ts`の差分・テスト結果を確認し、実際に数ケース再現確認した。
- 未コミット。前回のfriends×unlimited対応分(CLAUDE.md/data/activities.csv・json/scripts/generate-catalog.ts)と合わせてレビュー待ち。
